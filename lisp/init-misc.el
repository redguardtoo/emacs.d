...
If no region is selected, `kill-ring' or clipboard is used instead."
  ...
  (let* (rlt-buf
    ...
    (when (and fa (file-exists-p fa) fb (file-exists-p fb))
      ...
      (cond
       ((executable-find "git")
        (setq cmd (format "git diff --no-index --histogram --ignore-space-change --ignore-all-space --ignore-blank-lines \"%s\" \"%s\"" fa fb)))

       ((executable-find diff-command)
        (setq cmd (format "%s -Nabur \"%s\" \"%s\"" diff-command fa fb))))
