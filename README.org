...
- Only GUI Emacs can display image
...
=shr= supports COLOR but =w3m= not.

But my =w3m= based utilities can open built in video/audio/image. They can't be used until you replace =shr= with =w3m=. Anyway, your choice.
**** lua
...
- Used by =M-x my-grep= to grep text
- Install through OS package manager
I don't use =ripgrep= because it does not support [[not support https://github.com/BurntSushi/ripgrep/issues/225][decompression]].
* Usage
I *avoid overriding the original setup* of third party tool.

If I mention certain directories is used by certain tool, you can always confirm the information in the tool's original manual.

You can also =grep keyword= in the directory =~/.emacs.d/lisp= if you got any question on setup.
** Tutorial (OPTIONAL)
Basic knowledge of Linux/Unix is recommended except the tutorials . At least you should understand the words like "environment variable", "shell", "stdin", "stdout", "man", "info".
*** Basic tutorial
...
A generic grep tool =my-grep= is provided if you don't use =git=. Since =my-grep= is based on =counsel/ivy=, it also supports "multi-editing via Ivy". You could read [[https://sam217pa.github.io/2016/09/11/nuclear-power-editing-via-ivy-and-ag/][Nuclear weapon multi-editing via Ivy and Ag]] to get the idea.

The good news is my setup optimizes the multi-edit workflow. After =M-x my-grep= or pressing ",qq", you only need press =C-c C-o= once to use =wgrep-mode=, and you can edit text (for example, delete lines) in =wgrep-mode= without any extra key pressing.
** js2-mode
