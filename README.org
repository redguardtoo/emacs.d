...
- Tested with Emacs 29.1 on Linux/Windows10/macOS/MSYS2/Cygwin
...
- [[#faq][FAQ]]
  - [[#set-up-emacs-service][Set up Emacs service]]
  ...
  - [[#use-flycheck-or-flymake-syntax-check-code][Use Flycheck or Flymake syntax check code]]
...
* Checklist
- Emacs 29 or higher version is required
- Please read FAQ to disable [[http://www.vim.org][Vim]] key bindings
- Please read section "Key bindings" to learn key bindings
- Windows users had better download the version with dependencies bundled. For example, download "emacs-29.1-x86_64.zip" instead of "emacs-29.1-x86_64-no-deps.zip"
...
**** divsvgm
- Required by =M-x org-latex-preview=
**** latex
- Required by =M-x org-latex-preview=
- Run =brew install --cask mactex= on macOS
- Run =apt install texlive-full= or =apt install texlive-latex-base texlive-fonts-recommended texlive-latex-extra texlive-science cm-super= on Debian/Ubuntu Linux
**** identify from [[http://www.imagemagick.org/][ImageMagick]]
- Required by =org-mode= to export org file to odt containing images
**** zip and unzip
...
**** aider
- AI pair programming in your terminal. It's required by [[https://github.com/tninja/aider.el][aider.el]]
- Install [[http://pip.readthedocs.org/en/stable/installing/][pip]] through OS package manager, then =pip install aider-install && aider-install=
- See [[https://aider.chat/docs/llms/ollama.html]] on how to set it up on ollama
...
- Optionally used by =M-x fastctags-grep= to search text in files
...
Python formatter used by [[https://github.com/radian-software/apheleia][apheleia-mode]] which is not enabled by default.
* Tutorial (OPTIONAL)
...
Run =M-x fastctags-find-tag-at-point= from =fastctags= to navigate code. It uses tags files created by ctags. Tags file will be automatically created/update when you start using =fastctags=.

Run =M-x fastctags-grep= to search text (grep) in project. Project root is automatically detected.
...
Code auto-completion works out of box by using tags file created by Ctags.
...
Use [[file:tools/start-emacs-daemon.sh]] to set up emacs service.
*** macOS
...
<plist version="1.0">
  <dict>
    ...
    <array>
      <string>/path/to/start-emacs-daemon.sh</string>
    </array>
...
#+end_src
Run =launchctl load ~/Library/LaunchAgents/org.gnu.emacs.daemon.plist && launchctl start org.gnu.emacs.daemon= to enable&start emacs daemon on macOS.
*** Linux
...
[Service]
Type=forking
ExecStart=/path/to/start-emacs-daemon.sh
ExecStop=/path/to/start-emacs-daemon.sh stop
...
#+end_src
Run =systemctl --user enable emacs.service && systemctl --user start emacs.service= to enable&start emacs daemon on Linux.
...
To navigate, =M-x fastctags-find-tag-at-point=.

To enable code auto-completion, =M-x fastctags-scan-code= at least once.

Optionally, you can add =(add-hook 'after-save-hook 'fastctags-virtual-update-tags)= into your =.emacs= to automatically update tags file.
...
You can =M-x my-load-theme= to switch themes.
...
** Use Flycheck or Flymake syntax check code
Both packages are available but not activated.
...
You can set =my-initial-evil-state-per-major-mode= to customize the initial evil state per major mode,
#+begin_src elisp
;; `diff-mode' uses Emacs original key bindings
(push '(diff-mode . emacs) my-initial-evil-state-per-major-mode)
#+end_src
...
To enable the virtual environment, add below code into =~/.custom.el=,
#+begin_src elisp
(let* ((venv-dir "/path/to/my-venv"))
  (when (and (file-exists-p venv-dir)
             (executable-find pyvenv-virtualenvwrapper-python))
    (pyvenv-activate venv-dir)))
#+end_src

After activating the virtual environment, you could backup its packages,
#+begin_src sh
source /path/to/my-venv/bin/activate && pip freeze > venv-requirements.txt && deactivate
#+end_src

Restore the packages,
#+begin_src sh
source /path/to/my-venv/bin/activate && pip install -r venv-requirements.txt && deactivate
#+end_src

You might want to modify =include-system-site-packages= in =/path/to/my-venv/pyvenv.cfg=
...
(when (and (display-graphic-p) (or my-linux-p my-macos-p))
...
** Emacs 28
Version 4.2 is the last version to support =Emacs 28=
Please use [[https://github.com/redguardtoo/emacs.d/archive/4.2.zip]] and [[https://github.com/redguardtoo/myelpa/archive/4.2.zip]].

** Emacs 27
Version 4.1 is the last version to support =Emacs 27=
Please use [[https://github.com/redguardtoo/emacs.d/archive/4.1.zip]] and [[https://github.com/redguardtoo/myelpa/archive/4.1.zip]].
