...
#+end_src
*** Use your own spell check solution
Insert below code to disable builtin =wucuo= before using =flyspell-mode= or =flyspell-prog-mode=,
#+begin_src elisp
(setq my-disable-wucuo t)
#+end_src
*** Interval of spell check and syntax check
This configuration is already optimized for low specification machines on Windows/Linux/macOS by using =lazyflymake= and =wucuo=.

But you could still squeeze more performance by decreasing the frequency of spell checking and programming syntax check.

The spell check is done by [[https://github.com/redguardtoo/wucuo][wucuo]] which is lightweight alternative of =flyspell-mode= and =flyspell-prog-mode=.
