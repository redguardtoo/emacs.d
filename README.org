...
- I intentionally make packages from [[http://melpa.org][melpa-unstable]] *uninstallable* by default. You can modify variable =melpa-include-packages= in =~/.emacs.d/lisp/init-elpa.el= to install packages from melpa-unstable
