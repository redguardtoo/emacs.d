...
- Read FAQ to disable default [[http://www.vim.org][Vim]] key bindings
...
- I do NOT re-define new key bindings, so original key bindings from Emacs/Plugins are still there
...
Uninstall any system Emacs packages. For example, run "apt-get autoremove emacs-w3m" on Debian/Ubuntu. It's because all packages will be placed at "~/.emacs.d" from now on.
