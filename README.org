...
- Windows users'd better use [[http://www.cygwin.com/][Cygwin]]. But the setup is still usable in native windows version if you set up environment variable HOME.
...
You can use *stable* version of my setup only. Stable version has [[https://github.com/redguardtoo/emacs.d/issues/169][been regression testd by volunteers]] from time to time. "cd ~/.emacs.d/;git checkout stable" to get the stable version.
