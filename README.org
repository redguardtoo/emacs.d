...
- Support Windows. You can find&grep files on Window with minimum setup
- Tested with Emacs 24.4, 24.5, 25.3, 26.1 on Linux/Windows/Cygwin/macOS/WSL(Windows Subsystem for Linux)
- Default key bindings and  directory layout is respected
...
* Checklist
- Emacs 26.1 is strongly recommended. It has better performance and easier to setup
...
- Most packages from [[http://melpa.org][MELPA]] are *invisible* but packages from [[https://stable.melpa.org][MELPA Stable]] are visible. You can modify variable =melpa-include-packages= in =~/.emacs.d/lisp/init-elpa.el= to install unstable packages
...
Someone reported that package default repository lik [[http://melpa.org]] is not responsive in China. No worries. So you could answer "YES" to the question "Switch to faster package repositories in China temporarily?" when firing Emacs. Please note after startup you could change variable =package-archives= in =init-elpa.el= to permanently switch to Chinese repositories.
...
*** Install
Most command line programs can be installed throug =default package manager=.
...
- I force the aspell's dictionary to "en_US" in =~/.emacs./lisp/init-spelling.el=. You can modify it.
