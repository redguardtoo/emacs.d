...
* Warning
- Newbies who want to start immediately should check the section "Install stable version in easiest way".
- Emacs newer than v24.3.93.1 is *NOT* supported.
...
- Windows users could use [[http://www.cygwin.com/][Cygwin]] to avoid installing GNU tools. But this setup is still usable in native windows version if you set up environment variable HOME.
- My priority is stable. So I whitelist packages from [[http://melpa.org][melpa]] (melpa-unstable, actually). By default packages from melpa-unstable are *invisible*. Modify variable "melpa-include-packages" in init-elpa.el if you need packages from melpa-unstable.
- In the bug report, please include details (OS, Emacs version ...) and the output of `emacs --debug-init`.
...
Tested with v23.4, v24.3.1, v24.3.93.1.

Please contact the original developer if you have any issue with some plugin. [[http://www.emacswiki.org/emacs/][EmacsWiki]] also can help.
...
"lisp/init-elpa.el" define how and what packages will be installed from [[http://elpa.gnu.org][elpa]]/[[http://marmalade-repo.org][marmalade]]/[[http://melpa.org][melpa]] by package manager.
