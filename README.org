...
- My priority is stable. So I whitelist packages from [[http://melpa.milkbox.net][melpa]] (melpa-unstable, actually). By default packages from melpa-unstable are *invisible*. Modify variable "melpa-include-packages" in init-elpa.el if you need packages from melpa-unstable.
