...
By default, yasnippet is *NOT* loaded because it takes too much Emacs startup time.

Instead of "M-x yas-expand", you can use my wrapper "M-x my-yas-expand" whose hotkey is "C-c k" globally and "M-j" in evil-insert-mode.

If you prefer loading yasnippet at startup, insert below lines anywhere (~/.emacs.d/init.el, ~/.custom.el, ... etc):
#+begin_src elisp
(require 'yasnippet)
(yas-global-mode 1)
#+end_src
