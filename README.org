...
Uninstall third party Emacs package which is not placed at =~/.emacs.d=. For example, run =apt-get autoremove emacs-w3m= on Debian/Ubuntu. Do nothing if you don't know what I mean.
...
Some users reported that package repositories ([[http://melpa.org]] for example) is not responsive in China.

So it's better for users in China to answer "YES" to the question "Switch to faster package repositories in China temporarily?" when using this setup for the first time. Please note the variable =package-archives= in =init-elpa.el= still need be changed to use different repository permanently.
