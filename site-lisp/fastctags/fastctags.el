...
;;
;;     (add-hook 'prog-mode-hook #'fastctags-auto-setup)
;;
...
;;
;; - Set `fastctags-code-complete-ignore-case-p' to ignore case when fetching candidates
;;
...
;;
;; - `fastctags-debug-info' for debugging.
;;
...
(defcustom fastctags-code-complete-ignore-case-p nil
...
(defcustom fastctags-code-complete-everywhere nil
  "Non-nil to offer code completions in comments&strings."
  :type '(choice (const :tag "Off" nil)
                 (const :tag "On" t)))

(defcustom fastctags-update-tags-file-interval 120
...
(defvar fastctags-code-nav-cached-candidates nil
...
(defcustom fastctags-code-nav-find-tag-name-function 'fastctags-code-nav-find-tag-name-default
...
It's used by `fastctags-code-nav-find-tag-name-default'."
  :group 'fastctags
  :type '(repeat sexp))
...
(defcustom fastctags-code-nav-candidates-optimize-limit 1024
...
(defvar fastctags-code-nav-tag-history nil "History of tag names.")
...
(defvar fastctags-code-nav-find-tag-candidates nil "Find tag candidate.")
...
(defmacro fastctags-candidates-by-character (character dict)
...
(defun fastctags-parse-tags (text &optional dict)
...
If it's nil, return a dictionary, or else return the existing dictionary."
  (let* ((start 0)
         (vim-regex "^\\([^!\f\t\n\r()=,; ]+\\)\t\\(.+\\)$")
         (case-fold-search fastctags-code-complete-ignore-case-p)
         tagname
         c)

    (when fastctags-debug (message "fastctags-parse-tags called"))
    ...
    (while (string-match vim-regex text start)
      ...
      (when (fastctags-tag-name-character-p c)
        ...
        (push tagname (fastctags-candidates-by-character c dict)))
...
(defun fastctags-all-completions (string collection)
  ...
  (let ((case-fold-search fastctags-code-complete-ignore-case-p))
...
(defun fastctags-fetch-by-first-char (c prefix tagname-dict)
  ...
  (let* ((rlt (fastctags-all-completions prefix (fastctags-candidates-by-character c tagname-dict))))
    (when fastctags-code-complete-ignore-case-p
      (let (c2 (offset (- ?a ?A)))
        (cond
        ...
        (when c2
          (setq rlt (nconc rlt (fastctags-all-completions prefix (fastctags-candidates-by-character c2 tagname-dict)))))))
    rlt))
...
This function return t if FILE is reloaded."
  (let* (raw-content
    ...
    (when fastctags-debug
      (message "fastctags-load-tags-file called => %s %s " file use-diff))
      ...
      (cond
       (use-diff
        ...
        (let (diff-output)
          ...
          ;; content in :diff-output has higher priority than :raw-content
          (fastctags-put :diff-output diff-output file-info)
          ;; compare old and new tags file, extract tag names from diff output which
          ;; should be merged with old tag names
          (setq tagname-dict
                (fastctags-parse-tags diff-output
       ...
       (t
        ...
        (setq tagname-dict (fastctags-parse-tags raw-content))
...
(defun fastctags-test-cached-candidates (prefix)
  "Test PREFIX in `fastctags-code-nav-cached-candidates'."
  (let* ((cands fastctags-code-nav-cached-candidates)
...
(defun fastctags-code-nav-candidates (prefix)
  "Get code navigation candidate with PREFIX."
  (when fastctags-debug
    (message "fastctags-code-nav-candidates called => %s" prefix))

  (when (and prefix (> (length prefix) 0))
    (let* ((completion-ignore-case fastctags-code-complete-ignore-case-p)
      ...
      (when (and tags-file (file-exists-p tags-file))
        ...
        (when (fastctags-load-tags-file tags-file nil)
          ;; invalidate cached candidates if any tags file is reloaded
          (setq fastctags-code-nav-cached-candidates nil))
        ...
        (cond
         ...
         ((and fastctags-code-nav-cached-candidates
               (fastctags-test-cached-candidates prefix))

          (let* ((cands (plist-get fastctags-code-nav-cached-candidates :cands)))
         ...
         (t
          ...
          (setq fastctags-code-nav-cached-candidates
...
(defun fastctags-completion-table (string pred action)
  ...
  (if (eq action 'metadata)
      `(metadata (category . fastctags)
                 (cycle-sort-function . identity)
                 (display-sort-function . identity))
    (let ((candidates (fastctags-code-nav-candidates string)))
      (complete-with-action action candidates string pred))))

;;;###autoload
(defun fastctags-completion-at-point ()
  "The native Completion At Point Function for fastctags."
  (when (or fastctags-code-complete-everywhere
...
(defun fastctags-code-nav-sort-candidates-maybe (cands strip-count current-file)
...
CURRENT-FILE is used to compare with candidate path."
  (let* ((ref (and current-file (fastctags--strip-path current-file strip-count))))
    (cond
     ...
     ((or (not ref)
          (not fastctags-code-nav-candidates-optimize-limit)
          (>= (length cands) fastctags-code-nav-candidates-optimize-limit))
      cands)
...
(defun fastctags-tags-file-size (tags-file)
...
(defmacro fastctags-get-one-candidate (tagname-re bound root-dir)
...
(defun fastctags-code-nav-extract-cands (tags-file tagname fuzzy)
  "Parse TAGS-FILE to find occurrences of TAGNAME using FUZZY algorithm (maybe)."
  (let* ((root-dir (file-name-directory tags-file))
         ...
         file-info)
    (when fastctags-debug
      (message "fastctags-code-nav-extract-cands called. tags-file=%s cached-file-size=%s tags-file-size=%s"
               tags-file
               (fastctags-tags-file-size tags-file)
    ...
    (when (and tags-file (file-exists-p tags-file)
               ;; tags file is smaller when being created because it's incomplete tag file
               ;; Do NOT load it
               (< (fastctags-tags-file-size tags-file)
                  (nth 7 (file-attributes tags-file))))
      (when fastctags-debug
        (message "Read file .... %s %s" (fastctags-tags-file-size tags-file) (nth 7 (file-attributes tags-file))))
      (fastctags-load-tags-file tags-file nil))

    (setq file-info (fastctags-get-tags-file-info tags-file))

    (when fastctags-debug
      (message "fastctags-code-nav-extract-cands called. tags-file=%s tagname=%s" tags-file tagname))
      ...
      (with-temp-buffer
        ...
        (let* ((case-fold-search fuzzy)
          ...
          (while (re-search-forward tagname nil t) ; find line start with tagname
            (beginning-of-line)
            (setq cand (fastctags-get-one-candidate tagname-re
                                                    (line-end-position)
                                                    root-dir))
...
(defun fastctags-code-nav-collect-cands (tagname fuzzy current-file &optional dir)
  ...
  (let* (rlt
         ...
         (cands (and tags-file (fastctags-code-nav-extract-cands tags-file
                                                                 tagname
                                                                 fuzzy))))

    (when fastctags-debug
      (message "fastctags-code-nav-collect-cands called. tags-file=%s cands=%s" tags-file cands))
    ;; current-file is used to calculated string distance.
    (setq rlt (fastctags-code-nav-sort-candidates-maybe cands 3 current-file))
    (when fastctags-extra-tags-files
      ...
      (dolist (file (ff-list-replace-env-vars fastctags-extra-tags-files))
        ...
        (when (setq cands (fastctags-code-nav-extract-cands file tagname fuzzy))
...
(defun fastctags-tagname-at-point ()
  ...
  (setq fastctags-last-tagname-at-point
        (or (fastctags-selected-str)
            (funcall fastctags-code-nav-find-tag-name-function))))
...
(defun fastctags-code-nav-remember (cand dir)
  "Remember CAND whose `default-directory' is DIR."
  (setq fastctags-code-nav-tag-history
        (cl-remove-if (lambda (s)
                        (string= (car cand) (car s)))
                      fastctags-code-nav-tag-history))
  (let* ((v (cdr cand)))
    (fastctags-put :tags-file-directory dir v)
    (push (cons (car cand) v) fastctags-code-nav-tag-history)))
  ...
  (let* ((dir (fastctags-tags-file-directory))
    ...
    (cond
     ((= 1 (length cands))
      ...
      (fastctags-code-nav-remember (nth 0 cands) dir)
     ...
     ((setq selected (completing-read (format "Find Tag (%s): "
                                      ...
                                      nil
                                      t))
      (fastctags-code-nav-remember (assoc selected cands) dir)
...
;;;###autoload
(defun fastctags-code-nav-find-tag-name-default ()
  "Find tag at point."
...
(defun fastctags-code-nav-find-tag-api (tagname fuzzy current-file)
  ...
  (let* ((time (current-time))
    ...
    (when fastctags-debug
      (message "fastctags-code-nav-find-tag-api called => tagname=%s fuzzy=%s dir%s current-file=%s"
               tagname
               fuzzy
               dir
               current-file))
    ...
    (cond
     ...
     ((not (setq fastctags-code-nav-find-tag-candidates
                 (fastctags-code-nav-collect-cands tagname fuzzy current-file dir)))
     ...
     (t
      ...
      (fastctags-open-tag-cand tagname fastctags-code-nav-find-tag-candidates time)))))

;;;###autoload
(defun fastctags-code-nav-find-tag ()
...
Step 2, user keeps filtering tags."
  ...
  (let* ((tagname (completing-read "Regex to match tag: "
                                   nil
                                   (or (fastctags-selected-str) ""))))
    (when (and tagname (not (string= tagname "")))
      (fastctags-code-nav-find-tag-api tagname t buffer-file-name))))

;;;###autoload
(defun fastctags-code-nav-find-tag-at-point ()
...
That's the known issue of Emacs Lisp.  The program itself is perfectly fine."
  ...
  (let* ((tagname (fastctags-tagname-at-point)))
    (cond
     (tagname
      (fastctags-code-nav-find-tag-api tagname nil buffer-file-name))
     (t
      (message "No tag at point")))))

;;;###autoload
(defun fastctags-code-nav-recent-tag ()
  "Find tag using tagname from `fastctags-code-nav-tag-history'."
  (interactive)
  (cond
   ((not fastctags-code-nav-tag-history)
    (message "`fastctags-code-nav-tag-history' is empty."))
   (t
    (let* ((dir (fastctags-tags-file-directory))
...
;;;###autoload
(defun fastctags-grep (&optional default-keyword hint root show-keyword-p)
...
If `fastctags-use-git-grep-p' is t, git grep is grep program."
  ...
  (let* ((text (or default-keyword (fastctags-read-keyword hint)))
    ...
    (when (and cands
               ...
               fastctags-code-nav-candidates-optimize-limit
